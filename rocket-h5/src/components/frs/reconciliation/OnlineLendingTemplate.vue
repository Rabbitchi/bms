...
<template>
  <div class="table_container">
      ...
      >
        <el-row>
          <el-col :span="24">
            ...
            <!--<el-form-item label="business channel:" prop="businessChannel">
              <el-select v-model="queryConditions.data.businessChannel" placeholder="please select">
                <el-option label="All" value=""></el-option>
                <el-option :label="item" :value="item" v-for="(item,index) in termList" :key="index"></el-option>
              </el-select>
            </el-form-item>-->
            <el-form-item label="business channel:" prop="businessChannel">
              <el-select
                v-model="queryConditions.data.businessChannel"
                filterable
                allow-create
                placeholder="please select"
              >
                <el-option
                  v-for="(item, index) in businessChannelList"
                  :key="index"
                  :label="item"
                  :value="item"
                ></el-option>
    ...
    <div class="table_list">
      ...
      />
      <!--<p class="bottomTip">
        Total <span class="bold">{{ receiveOrderSum }}</span> orders,total due-repayment amount N <span class="bold">{{ receiveTotalAmount }}</span>.
      </p>-->
    </div>
    <!--导出弹窗-->
    <needFieldsDialog
...
export default {
  ...
  watch: {
    ...
    duedayTimeRange: function () {
      this.queryConditions.data.dueDateStart =
        this.duedayTimeRange && this.duedayTimeRange[0]
          ? this.duedayTimeRange[0]
          : ''
      this.queryConditions.data.dueDateEnd =
        this.duedayTimeRange && this.duedayTimeRange[1]
          ? this.duedayTimeRange[1]
          : ''
    }
  },
  created() {
    //如果路由带参，需要切换到对应的tab
    if (this.$route.query.productType && this.$route.query.businessDate) {
      this.timeRange.push(this.$route.query.businessDate)
      this.timeRange.push(this.$route.query.businessDate)
      this.duedayTimeRange.push(getPreDateTime())
      this.duedayTimeRange.push(getPreDateTime())
    } else {
      //给businessDate赋默认值
  ...
  mounted() {
    this.queryTermList()
    this.queryBusinessChannel()
  },
  methods: {
    //查询  businessChannel
    queryBusinessChannel() {
      let _this = this
      let reqData = {
        page: {
          pageNum: 1,
      ...
      businessChannelQuery(reqData)
        .then((res) => {
          _this.businessChannelList = res.data
        })
        .catch((e) => {
          console.log('error')
        })
    ...
    getListItem() {
      ...
      frsLoanSumQuery(_this.queryConditions)
        .then((res) => {
          ...
          if (res.data.receiveTotalAmount) {
            _this.receiveTotalAmount = res.data.receiveTotalAmount
          }
          if (res.data.receiveOrderSum) {
            _this.receiveOrderSum = res.data.receiveOrderSum
          }
