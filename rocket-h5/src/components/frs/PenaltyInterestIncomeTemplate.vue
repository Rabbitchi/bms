...
<template>
  <div class="table_container">
    ...
    <div class="table_list">
      ...
      >
        <el-table-column label="business Date" align="center">
        ...
        <el-table-column label="Overdue Interest" align="center">
          <template slot-scope="scope">
            <span>{{ scope.row.overdueInterest }}</span>
          </template>
        </el-table-column>
        <el-table-column label="transaction ID" align="center">
          <template slot-scope="scope">
...
import {
  overdueInterestDetailQuery,
  overdueInterestDetailExport
} from '@/api/frs/penalty-interest-income.js'
...
export default {
  ...
  data() {
    return {
      ...
      filterTemplateHeader: [
        ...
        { name: 'overdueInterest', value: 'overdueInterest', isRequired: false }
      ],
      //时间控件修改，限制只能选同一年
      minDate: '',
      maxDate: '',
  ...
  methods: {
    ...
    exportExcel() {
      ...
      // 导出
      overdueInterestDetailExport(_this.queryConditions)
        .then((res) => {
          if (res.data.status.code === 0) {
            ...
            //文件名
            _this.Filename =
              'Penalty Interest Income Details' +
              '(' +
              _this.productType +
              ')' +
    ...
    getListItem() {
      ...
      // 分页逻辑
      overdueInterestDetailQuery(_this.queryConditions)
        .then((res) => {
